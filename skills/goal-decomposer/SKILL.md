...
- `house_ux_rules[]`（互動/UX 慣例，跨品牌 — ADR-009）：對 **UI 型 goal**（產出顧客可見視覺面 / 後台頁 / 表單 / list / 生成流程），把命中 `emits_requirement` 的 UX 規則**烤成驗收條件**。重點幾條：
  - `screenshot-before-ship`：凡產出顧客可見視覺面的 goal，驗收**必含**「build 後截圖（375/768/1280）+ 自評 floor-guard + 把畫面攤給人做最終判」。scorecard 驗「截圖**產出且 surface**」（機械可驗）；**視覺品味的好壞判給人，executor / scorecard 不當品味裁判**（agent 是醜的地板守門員、人是 80 分裁判）。
  - `anti-slop-checklist` / `break-sparseness-rhythm` / `blocked-state-disclosure` / `multi-entry-feature-parity` 等命中的，寫成該 goal 的**負向驗收**（testable「別…」：別單欄文字堆、別只放 1 張圖、別把唯一動作藏成小灰字、別讓多入口漂移）。
  - 值為品牌專屬的（色票 / 字 / 形）**不在此烤死**——那走 `house_aesthetics` / 設計模子（per-brand），本層只烤「機制型」UX 慣例。
  - 與 archetype baseline（步驟七自檢）互補：baseline 管「該有的面有沒有」，house_ux_rules 管「那些面長得能不能用」。
...
| Audit | remediation-goals.md + project-fingerprint.md | goal 接到既有程式碼上；每個 goal 加「現狀」區塊 + 回歸保護驗收項（不得弄壞 fingerprint 中既有的 routes/介面）。**讀 remediation 的 `triage`/`blast_radius`（Audit Mode A7）：只排程 `triage=fix-now`；`blast_radius=high`（安全·RLS·GRANT·金流）的 goal 寫「執行前人看 SQL 才 apply」前置裁決 + opus tier → executor 跑到先輸出 SQL、走 BLOCKED protocol 回人，不自動 apply** |
...
- [ ] **house_ux_rules 覆蓋**（步驟零之二讀到 `house_ux_rules` 且專案有 UI 面時 — ADR-009）：每個 UI 型 goal 的驗收已烤進命中的 UX 規則；凡產出顧客可見視覺面的 goal 至少帶 `screenshot-before-ship`（截圖 → 人做 80 判）驗收。無 house-profile 的公開使用者本項略過（中性預設、照常跑）
