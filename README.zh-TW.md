...
1. **暗區偵測**——把軟體規格系統性遺漏的維度做成結構化清單（11 個基準維度 + 五種產品類型擴充包），由 5–7 輪問答引擎驅動，全程追蹤已知（亮區）與未知（暗區）。
...
│   暗區追蹤 · 衝突處理協議 · 重複造輪子偵測                     │
│   邊界掃描 · 矛盾掃描 · 未定值鐵律（步驟七之三）               │
│   旅程覆蓋檢查 · enterprise 外部治理 SKILL 推薦               │
│   handoff 需 boundary_scan done                             │
├─────────────────────────────────────────────────────────────┤
│ 第二層 · goal-decomposer（skill，已完成）                     │
│   規格 ── 依賴推導   ──► goal-graph.json                     │
│        ── 裁決前置   ──► goals/G*.md（自包含）                │
│        ── 介面凍結   ──► contracts/C*.md                     │
│   接線矩陣（孤兒介面＝編譯錯誤）· G0/G-FINAL 必產結構性 goal   │
│   跨端驗收拆半 · 無上下文盲審（步驟七之二）                    │
├─────────────────────────────────────────────────────────────┤
│ 第三層 · 執行層（任何模型）                                    │
...
spec-sonar 是 AI 開發工具鏈的**收斂＋分解層**（第一、二層）——五個公開 repo 加上作者私有的跨專案知識庫，總圖見 [specmit/ECOSYSTEM.md](https://github.com/dragon375014/specmit/blob/main/ECOSYSTEM.md)。

**一鍵安裝**（把所有工具裝到正確位置）：
```bash
npx specmit init
```
裝好後在 Claude Code 說「跑完整管線」，即可執行整條 pipeline。

- **下游**——[`goal-workflow-designer`](https://github.com/dragon375014/goal-workflow-designer) 負責單一任務塑形軸：其 `goal` skill 定義本 repo goal 檔沿用的五元素格式；`workflow-shaper` 處理同質批次 fan-out（同一檢查跑 N 個單位），不在本 repo 範圍內。
- **執行期**——[`specmit`](https://github.com/dragon375014/specmit) 執行分解後的 goal 圖；[`claude-skills-governance-meta`](https://github.com/dragon375014/claude-skills-governance-meta) 看守執行模型；[`agent-work-board`](https://github.com/dragon375014/agent-work-board) 協調平行 session。

## 語言說明

兩份 SKILL.md 目前以**繁體中文**撰寫——繁中讀者可直接閱讀與修改，不需對照翻譯。這也不影響跨語言使用：Claude 依 skill 邏輯運作並以用戶的語言回應（STATE 帶 `lang` 欄位）。英文原生 skill 檔已列入路線圖——歡迎貢獻。
...
- [x] `goal-graph.json` 執行器（自動按批次分派給模型）——已由 [specmit](https://github.com/dragon375014/specmit) 實現
