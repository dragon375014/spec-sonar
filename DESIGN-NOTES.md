...
## ADR-009｜UX 軸：goal-decomposer 讀 house_ux_rules + 視覺驗收烤進 UI goal（2026-06-29，已實作）

**狀態：** 已實作——`goal-decomposer/SKILL.md` 步驟零之二（house-profile 讀取多吃 `house_ux_rules[]`）+ 步驟七自檢加「house_ux_rules 覆蓋」。是 ADR-002（archetype 完備性）的姊妹：archetype 管「該有的**面**有沒有」，本案管「那些面**長得能不能用**」。

### 背景 / 問題
ADR-002 補了「該有的面有沒有」（archetype baseline），但留一個洞：**面存在、可達、甚至填了料，仍可以醜得沒人想用**。實測（旗艦 9453 + 體系自省）：管線每個閘門都建在文字/程式碼上（goal / 契約 / grep / build / git diff），**沒有任何環節把畫面 render 出來用眼睛看**——「pipeline is blind」。結果 UI goal 可以 100% verified（seam 全接、build 全綠）同時畫面醜，因為沒有任何閘門在測量視覺品質。

同時診斷出兩件事：(1) 消費者本地早有一支「視覺品質閘門」skill（CS `ui-presentation`，2026-06-14 粗糙面板事故後做），含 anti-slop checklist + mockup-before-build + **screenshot-before-ship**——但它 project-local、沒接進管線。(2) 這類 UX 慣例（跨品牌通用，如 blocked-state 揭露 / 多入口同契約 / 破稀疏節奏）與**品牌視覺品味**（per-brand：色/字/形）是**兩種東西**，混在一起會把品牌色當通用預設（同把 `loss_rate 12%` 當全域的錯）。

### 決策
**沿用 ADR-003 的能力閘 + 公私法則，加一個 UX 軸**：
1. **值（私有）**：house-profile 多一個陣列 `house_ux_rules[]`（跨品牌通用的互動/UX 慣例，每條同 `house_defaults` 形狀：`emits_requirement` / `default` / `source` / `confidence`）。品牌視覺值**不在這**（走 per-brand 設計模子 / `house_aesthetics`）。
2. **機制（公開 goal-decomposer）**：步驟零之二讀 house-profile 時**多吃 `house_ux_rules[]`**——對 UI 型 goal，把命中的 UX 規則**烤成驗收條件**。其中 `screenshot-before-ship` 把「視覺閘」變成 **goal 驗收**（截圖 → 人做 80 判），**不需要改 specmit 引擎**：閘走契約層、不走 runner step。
3. **品味判給人，不給 agent**：scorecard 只驗「截圖產出且 surface」（機械可驗）；視覺好壞由人裁決。agent 是「醜的地板守門員」（rubric 防地板 40→~70），**穩定 80 必須人在環內**（不賣「全自動 80 分 UI」）。
4. **能力閘 + 優雅降級（同 ADR-001/003）**：公開使用者無 house-profile → 中性預設、本步略過、照常跑。

### 為什麼是這個形狀（關鍵）
- **視覺閘走 goal 驗收、不走引擎**：把 screenshot-before-ship 烤進 UI goal 的 acceptance，executor 滿足它、scorecard 驗證據——管線引擎零改動、零風險。比在 runner 加特殊 step 乾淨。
- **公私分離第六次**（ADR-001/003/004/005/008/本案）：機制公開（讀 house_ux_rules）、值私有（規則內容）、品牌視覺更私有（模子）。三層：`house_defaults`（工程利齒）/ `house_ux_rules`（通用互動 UX）/ 模子（per-brand 視覺）。
- **N-紀律**：讀取機制 N≥2-clean（同 house_defaults）；`screenshot-before-ship` N≥2（CS ui-presentation + 9453 兩專案）。其餘規則的可信度由私有 `confidence` 欄標、使用者自決；公開機制不 hardcode 任何單例規則。

### 影響面
- spec-sonar：`goal-decomposer/SKILL.md`（步驟零之二 +1 bullet、步驟七 +1 自檢）。**不改 specmit 引擎**、不改 dark-zone-baseline（archetype 軸已在）。
- 私有（不在本 repo）：house-profile `house_ux_rules[]`（執行者維護）。

### ⚠️ 不要改回去
- **不要**把品牌視覺值（色/字/形）塞進公開機制或 house_ux_rules——那是 per-brand 模子，混進來＝把單一品牌烤死進通用層。
- **不要**讓 agent / scorecard 當視覺品味裁判——它只驗「截圖產出且 surface」，品味判給人。把「截圖閘」當成「自動產生漂亮 UI」會失望。
- **不要**為了視覺閘去改 specmit runner——閘走 goal 驗收即可，引擎零風險是刻意選擇。
- **不要**讓「沒有 house_ux_rules」變報錯——公開使用者中性預設是正常路徑。

### 參考
- 值的家 + 完整 ≈15 條清單 + 三層分類：執行者私有（公開 repo 不需知道）。
- 證據鏈：CS `文檔/設計地基模子/00_三層分類與house_ux_rules候選_2026-06-29.md`（#8 同款唯讀挖掘）；旗艦 9453 視覺迴路實證（screenshot-before-ship + 破稀疏節奏過 80）。
- prior-art：消費者本地 `ui-presentation` skill 是本案的 project-local 前身；本案把它的通用核心升到管線層。
